...
function LodgingView({ guests, workspaceName, loading, loadError, viewMode, onViewModeChange, canEdit, onRetry, onEdit }: {
  ...
  const allocatedFamilies = guests.filter(hasAssignedRooms).length
      ...
      {loading ? <div className="guest-loading"><LoaderCircle className="spin" size={22} /> Loading lodging…</div>
        : loadError ? <div className="guest-state error-state"><h2>Couldn’t load lodging</h2><p>Check your connection or workspace access, then try again.</p><button className="secondary-button" onClick={onRetry}>Try again</button></div>
          : guests.length === 0 ? <div className="guest-state"><div className="empty-mark"><BedDouble size={23} /></div><h2>No families yet</h2><p>Add families in Guests, then record their stay and room details here.</p></div>
            : visibleGuests.length === 0 ? <div className="guest-state compact-state"><h2>No families in this filter</h2><p>There are no families with {roomFilter === 'allocated' ? 'allocated' : 'unallocated'} rooms.</p><button className="text-button" onClick={() => setRoomFilter('all')}>Show all families</button></div>
              : viewMode === 'spreadsheet' ? <div className="lodging-table-scroll" role="region" aria-label="Lodging spreadsheet" tabIndex={0}><table className="lodging-table"><thead><tr><th>Family</th><th>Guests</th><th>Total rooms</th><th>Allotted room numbers</th>{canEdit && <th className="lodging-action-heading"><span className="sr-only">Edit</span></th>}</tr></thead><tbody>{visibleGuests.map(guest => <tr key={guest.id}><th scope="row"><strong>{guest.family_name}</strong></th><td>{guest.guest_count}</td><td>{guest.room_count ?? <span className="sheet-muted">Not set</span>}</td><td>{guest.assigned_room_numbers?.filter(room => room.trim()).join(', ') || <span className="sheet-muted">Not allotted</span>}</td>{canEdit && <td className="lodging-action"><button className="sheet-edit-button" aria-label={`Edit lodging for ${guest.family_name}`} title="Edit lodging" onClick={() => onEdit(guest)}><Edit3 size={16} /></button></td>}</tr>)}</tbody></table><p className="spreadsheet-hint">Scroll sideways to see more columns.</p></div>
